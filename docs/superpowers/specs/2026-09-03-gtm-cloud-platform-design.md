...
| C8 | Ansible runs from WSL and drives the Windows gcloud through interop with `CLOUDSDK_CONFIG` | 85 % | [probe] `gcloud config list` + `auth list` in WSL | playbook cannot authenticate | Task 3 runs `foundation.yml --tags preflight`; fallback = Cloud Shell (documented) |
| C9 | `bq` also works through interop for the dataset GRANT | 60 % | not probed | GRANT task fails | Task 3 probes `bq version` in WSL; fallback = documented one-liner for Windows `bq` (PS quoting rule from gcp-auth-invariants) |
| C10 | Ansible `ini` lookup (`type=properties`) parses the repo `.env` (comments + `KEY=value`) | 70 % | [recall] lookup docs | empty token → broken secret version | Task 3 verifies with `ansible localhost -m debug` against `.env.example` (never the real file) |
